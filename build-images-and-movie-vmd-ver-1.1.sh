#!/bin/bash
mkdir -p profilin-1-movie

for name in `ls -d *P* | grep -v tcl | grep -v rmsd | cut -d' ' -f 9`
do
...
mol new $name/pro.${name//[!0-9]/}.psf
mol addfile $name/wrapped_trajectories/wrapped/wrapped-${name//[!0-9]/}.dcd type dcd first 0 last -1 step 10 filebonds 1 autobonds 1 waitfor all
...
#cd profilin-1-movie
ppmtompeg $name/$name-movie.prm
mv $name.mpeg profilin-1-movie
xdg-open profilin-1-movie/$name.mpeg
done
